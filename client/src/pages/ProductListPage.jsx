        ...
        <div className=' min-h-[88vh] max-h-[88vh] overflow-y-scroll  grid gap-1 shadow-md scrollbarCustom bg-white py-2'>
          {
            DisplaySubCatory.map((s) => {
              const baseCategory = s?.category?.[0]
              const link = baseCategory
                ? `/${valideURLConvert(baseCategory.name)}-${baseCategory._id}/${valideURLConvert(s.name)}-${s._id}`
                : "#"
              return (
                <Link
                  key={s._id}
                  ...
                  ${subCategoryId === s._id ? "bg-pink-100" : ""}
                `}
                >
